...
`)
	...
	var runCancelledByStatusStateMatrix = PipelineRunState{{
		...
		RunObjects:     []v1.RunObject{withRunCancelled(withRunRetries(newRun(runs[0])))},
		CustomTask:     true,
		ResolvedTask: &resources.ResolvedTask{
			TaskSpec: &task.Spec,
		},
	...
	var runCancelledBySpecStateMatrix = PipelineRunState{{
		PipelineTask:   &pts[20], // 2 retries needed
		RunObjectNames: []string{"pipelinerun-mytask1"},
		RunObjects:     []v1.RunObject{withRunCancelledBySpec(withRunRetries(newRun(runs[0])))},
		CustomTask:     true,
		ResolvedTask: &resources.ResolvedTask{
			TaskSpec: &task.Spec,
		},
	...
	var runSucceededStateMatrix = PipelineRunState{{
		PipelineTask:   &pts[20], // 2 retries needed
		RunObjectNames: []string{"pipelinerun-mytask1"},
		RunObjects:     []v1.RunObject{makeRunSucceeded(runs[0])},
		CustomTask:     true,
		ResolvedTask: &resources.ResolvedTask{
			TaskSpec: &task.Spec,
		},
	...
	var runExpectedStateMatrix = PipelineRunState{{
		PipelineTask:   &pts[20], // 2 retries needed
		RunObjectNames: []string{"pipelinerun-mytask1"},
		RunObjects:     []v1.RunObject{failedV1alpha1Run},
		CustomTask:     true,
		ResolvedTask: &resources.ResolvedTask{
			TaskSpec: &task.Spec,
		},
...
func TestDAGExecutionQueueSequentialRuns(t *testing.T) {
	...
	tcs := []struct {
		name       string
		firstRun   *v1.CustomRun
		secondRun  *v1.CustomRun
		specStatus v1.PipelineRunSpecStatus
		wantFirst  bool
		wantSecond bool
	}{{
	...
	var timedOutRun = PipelineRunState{{
		...
		RunObject: &v1.CustomRun{
			Spec: v1.CustomRunSpec{
				StatusMessage: v1.CustomRunCancelledByPipelineTimeoutMsg,
			},
			Status: v1.CustomRunStatus{
				Status: duckv1.Status{Conditions: []apis.Condition{{
					Type:   apis.ConditionSucceeded,
...
func TestPipelineRunState_GetResultsFuncs(t *testing.T) {
	...
	expectedRunResults := map[string][]v1.CustomRunResult{
...
func customRunWithName(name string) *v1.CustomRun {
	return &v1.CustomRun{
		ObjectMeta: metav1.ObjectMeta{
			Name: name,
		},
