...
func (state PipelineRunState) GetRunsResults() map[string][]v1.CustomRunResult {
	results := make(map[string][]v1.CustomRunResult)
	for _, rpt := range state {
		if !rpt.IsCustomTask() {
			continue
		}
		...
		if rpt.RunObject != nil {
			...
			case *v1alpha1.Run:
				for _, origRes := range r.Status.Results {
					results[rpt.PipelineTask.Name] = append(results[rpt.PipelineTask.Name], v1.CustomRunResult{
						Name:  origRes.Name,
						Value: origRes.Value,
					})
				}
...
func (t *ResolvedPipelineTask) getChildRefForRun(runObj v1.RunObject) v1.ChildStatusReference {
	apiVersion := ""
	kind := ""
	switch runObj.(type) {
	case *v1.CustomRun:
...
//
// Note: This function only detects if v1alpha1.Run is retriable. For v1.CustomRun and TaskRuns,
