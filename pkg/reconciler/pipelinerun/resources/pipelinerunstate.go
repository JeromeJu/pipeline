...
func (state PipelineRunState) GetTaskRunsResults() map[string][]v1.TaskRunResult {
	...
	for _, rpt := range state {
		...
		if rpt.TaskRun != nil {
			results[rpt.PipelineTask.Name] = rpt.TaskRun.Status.Results
		}
	}
	return results
}
...
func (t *ResolvedPipelineTask) getChildRefForRun(runObj v1beta1.RunObject) v1.ChildStatusReference {
	apiVersion := ""
	kind := ""
	switch runObj.(type) {
	case *v1beta1.CustomRun:
